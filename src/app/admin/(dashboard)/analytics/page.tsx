    ...
    <div className="max-w-6xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-dashed border-[#e5beb6] pb-6">
        ...
        <div className="flex items-center gap-2 bg-[#f4f4f1] border border-[#e5beb6] p-1 pl-3">
          <Calendar className="size-[18px] text-[#5c403a]" />
          ...
          return (
            <div key={m.label} className="bg-white border border-[#e5e1e9] p-5 pt-8 flex flex-col">
              <div className="flex justify-between items-start mb-4">
                ...
                <Icon className="size-5 text-[#b51c00]" />
              </div>
      ...
      <div className="bg-white border border-[#e5e1e9] p-6">
        ...
        <div className="relative h-64 w-full bg-[#FAFAF7] border border-[#e5beb6] flex items-end p-4 gap-2">
          ...
          {[40, 50, 30, 70, 85, 60, 90].map((clickPct, i) => (
            <div key={i} className="flex-1 flex items-end gap-1 h-full">
              <div className="w-1/2 bg-[#b51c00] transition-all hover:opacity-80" style={{ height: `${clickPct}%` }} />
              <div className="w-1/2 bg-[#FFC93C] transition-all hover:opacity-80" style={{ height: `${Math.round(clickPct * 0.45)}%` }} />
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white border border-[#e5e1e9] overflow-hidden">
          <div className="p-4 border-b border-[#e5e1e9] bg-[#f4f4f1]">
            <h3 className="font-sans text-[20px] leading-[28px] font-bold uppercase">Top Traffic Sources</h3>
          </div>
          <ul className="flex flex-col">
            {topSources.map((src) => (
              <li key={src.rank} className="flex items-center justify-between p-4 border-b border-dashed border-[#e5e1e9] hover:bg-[#FAFAF7] transition-colors hover:border-l-4 hover:border-[#b51c00]">
                <div className="flex items-center gap-3">
                  <span className="font-mono text-[13px] font-bold text-[#1a1c1b] w-4">{src.rank}.</span>
                  <span className="font-sans text-[16px] leading-[24px] text-[#1a1c1b]">{src.name}</span>
                </div>
                <div className="flex items-center gap-4">
                  <div className="w-24 h-2 bg-[#e2e3e0] rounded-full overflow-hidden hidden sm:block">
                    <div className="h-full bg-[#b51c00]" style={{ width: `${src.pct}%` }} />
                  </div>
                  <span className="font-mono text-[13px] font-bold text-[#1a1c1b] w-12 text-right">{src.pct}%</span>
                </div>
              </li>
            ))}
          </ul>
        ...
        <div className="bg-white border border-[#e5e1e9] overflow-hidden flex flex-col">
          <div className="p-4 border-b border-[#e5e1e9] bg-[#f4f4f1] flex justify-between items-center">
            ...
            <MapPin className="size-5 text-[#5c403a]" />
          </div>
